Процедура ВыполнитьОбработкуБотов() Экспорт 
	...
	Параметры = Обработки.RabbitMQ.ПолучитьПараметры();
	
	Обработки.RabbitMQ.ПроверитьПодключениеСервер(Новый УникальныйИдентификатор());
	//Обработки.RabbitMQ.СозданиеТочкиИОчередиСервер();
	ПараметрыДанных = Обработки.RabbitMQ.ПрочитатьСообщениеСервер();
	Если не ПустаяСтрока(ПараметрыДанных.ОтветноеСообщение) Тогда 
		Структура = JsonВСтруктуру(ПараметрыДанных.ОтветноеСообщение);  
		ЗаказОбъект = Документы.Заказ.СоздатьДокумент();
		ЗаказОбъект.Дата = xmlЗначение(тип("дата"),Структура.date); 
		ЗаказОбъект.ID = Структура.ID; 
		ЗаказОбъект.Покупатель =  Справочники.Контрагенты.НайтиПоРеквизиту("ИНН",Структура.inn); 
		ЗаказОбъект.Организация  = Справочники.Организации.НайтиПоКоду("000000001");  
		ЗаказОбъект.СостояниеЗаказа = Перечисления.СостоянияЗаказов.ВРаботе;
		ЗаказОбъект.ВидЦен = Справочники.ВидыЦен.НайтиПоНаименованию("Оптовая");
		Для Каждого СтрокаТаблицы из Структура.items Цикл 
			НоваяСтрока = ЗаказОбъект.Товары.Добавить();
			НоваяСтрока.Товар = Справочники.Товары.НайтиПоКоду(СтрокаТаблицы.id);
...
Процедура НеотработанныеЗаказы() Экспорт
	...
	Запрос.УстановитьПараметр("Номер", НачалоДня(ТекущаяДата()) - ПериодПроверки * 24 * 60 * 60);
